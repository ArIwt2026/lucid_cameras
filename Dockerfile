...
RUN git clone --depth 1 --branch v1.3 https://github.com/lucidvisionlabs/arena_camera_ros2.git /tmp/arena_camera_ros2 \
    ...
    && sed -i 's/Arena::SetNodeValue<double>(nodemap, "Gain", gain_);/Arena::SetNodeValue<GenICam::gcstring>(nodemap, "GainAuto", "Off"); Arena::SetNodeValue<double>(nodemap, "Gain", gain_); auto gain_auto_readback = Arena::GetNodeValue<GenICam::gcstring>(nodemap, "GainAuto"); auto gain_readback = Arena::GetNodeValue<double>(nodemap, "Gain"); log_info(std::string("GainAuto readback: ") + gain_auto_readback.c_str() + std::string(", Gain readback: ") + std::to_string(gain_readback));/' src/arena_camera_node/src/ArenaCameraNode.cpp \
